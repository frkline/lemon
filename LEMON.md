...
1. **Plan gate.** Fresh sessions launch `--permission-mode plan`. Claude proposes a plan via ExitPlanMode; an `ExitPlanMode` hook (`.claude/settings.json`, written per-worktree) copies the plan to `/tmp/lemon-plan-{slug}.md`. `WorktreeRunner.planGatePhase` posts the plan to the issue (🍋 Waiting, `.planReview`) and parks. The user approves in the popover / via `approve_gate` MCP / remote control → `Orchestrator.resolveGate` send-keys **"1"** ("Yes, and use auto mode") + writes `/tmp/lemon-gate-{slug}`, and the **same session** continues into the build.
