...
- **Stay in the session directory** (`/tmp/lemon-{slug}/`) — work happens there (it may hold several repo worktrees in multi-repo workspaces; cross-repo navigation is fine), and so do dev servers, builds, and infra. Toolchains reading their own home config/caches (`~/.npm`, `~/Library/Caches`, `~/.docker`, git/gh/ssh) is fine. **Don't read or recursively scan the home dir's personal-data folders** (`~/Downloads`, `~/Desktop`, `~/Documents`, `~/Pictures`, `~/Music`, `~/Movies`) and never run an unrooted `find`/`grep`/`rg` from `~` or `/` — that trips macOS privacy (TCC) prompts attributed to Lemon (#86). Lemon ships **no** Downloads/Photos/Music `NS*UsageDescription`, so such access is denied outright; containers (#15) make this structural.
